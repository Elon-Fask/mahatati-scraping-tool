...
def scraper_data_gasoil10_price(driver):
    ...
    while True:
        ...
        swipe_down(driver)
        if driver.find_element(AppiumBy.ID, "com.mahatati.mag:id/stationdistanceTxtView").text == "‎ 19 KM":
            break
    ...
    df.to_csv("mahatati_data_gasoil10_WINXO.csv", index=False)
...
def scraper_data_supercarburant_price(driver):
    ...
    df.to_csv("mahatati_data_supercarburant_WINXO.csv", index=False)
